# Generated by Django 2.0.3 on 2018-07-28 02:57
...
def get_all_chapter_users(apps, schema_editor):
    ...
    for chapter_name in chapter_names:
        ...
        if not chapter_info[1]:
            continue
        get_chapter_data(chapter_info[1],
                         apps)


def get_chapter_data(chapter_key, apps):
    ...
    for sheet in sheets:
        # print(sheet.title)
        if "Membership" in sheet.title:
            sleep(1)
            users = sheet.get_all_records()
            for user in users:
                add_user_info(user, apps)
...
org_columns = [
    ("Professional/ Technical Orgs", "Officer (Pro/Tech)", 'pro'),
    ("Honor Orgs", "Officer (Honor)", 'hon'),
    ("Other Orgs", "Officer (Other)", 'oth'),
]


def add_user_info(user_row, apps):
    user = apps.get_model("users", "User")
    org = apps.get_model("users", "UserOrgParticipate")
    gpa = apps.get_model("users", "UserSemesterGPA")
    service = apps.get_model("users", "UserSemesterServiceHours")
    email = user_row.get("Email Address", None)
    name = user_row.get("Name", "")
    if email is None and name == "":
        print("No email or name:", user_row)
        return
    badge_number = user_row.get("Badge Number", None)
    if badge_number is None:
        print("No badge number", badge_number)
        return
    try:
        user_obj = user.objects.filter(Q(email=email) |
                                       Q(user_id=badge_number) |
                                       Q(name=name)).first()
    ...
    if user_obj is None:
        print(f"User does not exist: {email}")
        return
    for column in service_gpa_columns:
        value = 0
        user_val = user_row.get(column, None)
        if user_val is None:
    ...
    for org_column in org_columns:
        org_name_col, officer_col, org_type = org_column
        user_val = user_row.get(org_name_col, None)
        if user_val is None:
            print(f"{column} does not exist")
            continue
        if user_val:
            officer_val = user_row.get(officer_col, None)
            officer = False
            if officer_val:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('users', '0009_initial_alumni_data'),
    ]

    operations = [
        migrations.RunPython(get_all_chapter_users,
