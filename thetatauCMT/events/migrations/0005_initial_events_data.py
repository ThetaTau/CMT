# Generated by Django 2.0.3 on 2018-06-24 03:00
import os
import datetime
...
def get_chapter_data(chapter_name, chapter_key, apps):
    ...
    for sheet in sheets:
        # print(sheet.title)
        if "Event" in sheet.title:
            sleep(1)
            events = sheet.get_all_records()
            for event in events:
                add_event(chapter_obj, event, apps)
...
def add_event(chapter_obj, event_row, apps):
    ...
    if event_row['Score']:
        try:
            score_obj = float(event_row['Score'])
        except ValueError:
            print(f"ERROR: score value {event_row['Score']}")
    guests_obj = 0
    if event_row['# Non- Members']:
        try:
            guests_obj = int(event_row['# Non- Members'])
        except ValueError:
            print(f"ERROR: guests value {event_row['# Non- Members']}")
    duration_obj = 0
    if event_row['Event Hours']:
        try:
            duration_obj = int(event_row['Event Hours'])
        except ValueError:
            print(f"ERROR: duration value {event_row['Event Hours']}")
    miles_obj = 0
    if event_row['MILES']:
        try:
    ...
    new_event = event(
        ...
        stem=event_row['STEM?'] == "Yes",  # True/False
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(get_all_chapter_events,
