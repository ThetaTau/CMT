...
class ObjectiveDetailView(LoginRequiredMixin, MultiFormsView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (self.object.restricted_co or self.object.restricted_ec) and not request.user.is_superuser:
            messages.add_message(
                request,
                messages.INFO,
                "You do not have permission to see this goal.",
            )
    ...
    def create_actions_form(self, **kwargs):
        ...
        if not actions:
            extra = 1
        factory = modelformset_factory(Action, form=ActionForm, **{"can_delete": True, "extra": extra})
        formset_kwargs = {
            "queryset": actions,
        }
...
class ObjectiveListView(LoginRequiredMixin, PagedFilteredTableView):
    ...
    def get_queryset(self, **kwargs):
        ...
        qs = qs.annotate(actions_count=Count("actions__complete", filter=Q(actions__complete=False)))
        return qs
